        ...
        public static class Road
        {
            ...
            public static class BigBlue
            {
                ...
                private static Tristrip[] CreateRailwithUV(Matrix4x4[] matrices, float height, float repetitionsRoadTexture, bool isLeftSide, bool isAlt)
                {
                    ...
                    var top = new Vector3(side, height, 0f);
                    var bottom = new Vector3(side, kCurbHeight, 0f);
                ...
                public static Tristrip[] RoadTrim(Matrix4x4[] matrices, GfzShapeRoad road, float segmentLength, bool isGfzCoordinateSpace, bool isAlt) {
                    var tristrips = MuteCity.TrimTex0(matrices, road, segmentLength, isGfzCoordinateSpace);
                    var x = isAlt ? -1 : 1;
            ...
            public static class PortTown
            {
                ...
                public static Tristrip[] CurbSlant(Matrix4x4[] matrices, GfzShapeRoad road, float segmentLength, bool isAlt)
                {
                    var tristrips = StandardCurbSlant(matrices);
                    float repetitionsRoadTexture = math.ceil(segmentLength / kLengthTrim);
                    var uvs = CreateTristripScaledUVs(tristrips, 2, repetitionsRoadTexture);
                    for (int i = 0; i < tristrips.Length; i++)
                    {
                        tristrips[i].tex0 = uvs[i];
                ...
                public static Tristrip[] LaneDividers(Matrix4x4[] matrices, GfzShapeRoad road, float segmentLength)
                    => MuteCity.LaneDividersTex0(matrices, road, segmentLength);
                ...
                public static Tristrip[] RoadRail(Matrix4x4[] matrices, GfzShapeRoad road, float segmentLength)
                {
                    var allTristrips = new List<Tristrip>();
                    ...
                    if (road.RailHeightLeft > 0f)
                    {
                        var railTristripsLeft = CreateRailwithUV(matrices, road.RailHeightLeft, repetitionsRoadTexture, true);
                        allTristrips.AddRange(railTristripsLeft);
                    }
                    ...
                    if (road.RailHeightRight > 0f)
                    {
                        var railTristripsLeft = CreateRailwithUV(matrices, road.RailHeightRight, repetitionsRoadTexture, false);
                        allTristrips.AddRange(railTristripsLeft);
                    }
                ...
                public static Tristrip[] RoadSideLow(Matrix4x4[] matrices, GfzShapeRoad road, float segmentLength, bool isAlt)
                {
                    ...
                    float repetitionsRoadTexture = math.ceil(segmentLength / kLengthRoadSides);
                    var uvs = CreateTristripScaledUVs(tristrips, -2, repetitionsRoadTexture);
                    for (int i = 0; i < tristrips.Length; i++)
                    {
                        tristrips[i].tex0 = uvs[i];
                ...
                public static Tristrip[] EndCaps(Matrix4x4[] matrices, GfzShapeRoad road, float segmentLength, bool isAlt)
                {
                    ...
                    if (!isContinuousFrom)
                    {
                        var matrix = matrices[0];
                        var uvRepsX = Mathf.Ceil(matrix.lossyScale.x / kEndCapTexStride);
                        var tristrips = StandardTrapezoidEndCapUVTex0(matrix, 0, kSideInset, kCurbHeight * -2f, kTrackThickness, Vector3.back, new Vector2(uvRepsX, -kEndCapTexHeight));
                    ...
                    if (!isContinuousTo)
                    {
                        var matrix = matrices[matrices.Length - 1];
                        var uvRepsX = Mathf.Ceil(matrix.lossyScale.x / kEndCapTexStride);
                        var tristrips = StandardTrapezoidEndCapUVTex0(matrix, 0, kSideInset, kCurbHeight * -2f, kTrackThickness, Vector3.forward, new Vector2(uvRepsX, -kEndCapTexHeight));
                ...
                //public static Tristrip[][] Shape(Matrix4x4[] matrices, GfzShapeRoad road, float maxTime, bool isGfzCoordinateSpace, bool isAlt)
                //{
                //    var tristrips = new Tristrip[][]
                //    {
                //        RoadTop(matrices, road, maxTime, isAlt),
                //        RoadBottom(matrices, road, maxTime, isAlt),
                //        TrimTex0(matrices, road, maxTime, isGfzCoordinateSpace),
                //        LightTex1(matrices, maxTime),
                //        LaneDividersTex0(matrices, road, maxTime),
                //        RailsTex2(matrices, road),
